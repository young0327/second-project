...
<link rel="stylesheet" type="text/css"
	href=" ${pageContext.request.contextPath}/resources/css/search.css?ver=1">
<title>AppV</title>
			...
			<c:choose>
				...
				<c:otherwise>
					<div class="col-sm-3 loginbox solo kr-font">
						<p class="kr-font" style="font-size: 20px;">${users.nick}님
							반갑습니다</p>
					</div>
				</c:otherwise>
			</c:choose>
					...
					<c:choose>
						<c:when test="${!empty searchName}">
							<div class="kr-font result-alert">
								<i class="fas fa-search"></i><span> '${searchName}'에 대한
									검색결과</span>
							</div>
						</c:when>
						<c:otherwise>
							<div class="kr-font result-alert">
								<i class="fas fa-search"></i><span> 검색어를 입력해 주세요.</span>
							</div>
						</c:otherwise>
					</c:choose>
						...
						<c:forEach items="${applist}" var="applist">
							<div class="result-box row">
								<div class="app-icon-img col-md-3">
									<img src="${applist.appicon}">
								</div>
								<div class="kr-font app-content-box col-md-9">
									<h5 class="kr-font">
										<a href="detail.do?appid=${applist.appid}"
											style="color: black;">${applist.appname}</a>
									</h5>
